            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="card p-6 lg:col-span-2">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h2 className="text-xl font-heading" style={{ color: 'var(--text-on-card)' }}>Configuration Files</h2>
                    </div>

                    <p className="mb-4 text-sm" style={{ color: 'var(--text-secondary)' }}>
                        Quick access to configuration files for advanced customization. Files are located in <code className="px-1 py-0.5 rounded" style={{ background: 'var(--bg-secondary)' }}>{dataPath.current || 'C:\\LocalDevine'}\\config</code>
                    </p>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <button
                            onClick={() => window.electronAPI?.openConfigFile('php.ini')}
                            className="button-secondary text-left flex items-center gap-3 p-4"
                        >
                            <span className="text-2xl">📝</span>
                            <div className="flex-1">
                                <div className="font-semibold" style={{ color: 'var(--text-on-card)' }}>php.ini</div>
                                <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>PHP Configuration</div>
                            </div>
                        </button>

                        <button
                            onClick={() => window.electronAPI?.openConfigFile('httpd.conf')}
                            className="button-secondary text-left flex items-center gap-3 p-4"
                        >
                            <span className="text-2xl">🌐</span>
                            <div className="flex-1">
                                <div className="font-semibold" style={{ color: 'var(--text-on-card)' }}>httpd.conf</div>
                                <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>Apache Configuration</div>
                            </div>
                        </button>

                        <button
                            onClick={() => window.electronAPI?.openConfigFile('config.json')}
                            className="button-secondary text-left flex items-center gap-3 p-4"
                        >
                            <span className="text-2xl">🔧</span>
                            <div className="flex-1">
                                <div className="font-semibold" style={{ color: 'var(--text-on-card)' }}>config.json</div>
                                <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>App Configuration</div>
                            </div>
                        </button>

                        <button
                            onClick={() => window.electronAPI?.openConfigFile('my.ini')}
                            className="button-secondary text-left flex items-center gap-3 p-4"
                        >
                            <span className="text-2xl">🗄️</span>
                            <div className="flex-1">
                                <div className="font-semibold" style={{ color: 'var(--text-on-card)' }}>my.ini</div>
                                <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>MariaDB Configuration</div>
                            </div>
                        </button>
                        ...
                        >
                            <span className="text-2xl">📁</span>
                            <div className="flex-1">
                                <div className="font-semibold" style={{ color: 'var(--text-on-card)' }}>Config Folder</div>
                                <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>Open all config files</div>
                            </div>
                        </button>
                    </div>
                    ...
                    <div className="mt-4 p-3 rounded-lg" style={{ background: 'var(--bg-secondary)' }}>
                        <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
                            💡 <strong>Tip:</strong> After editing configuration files, restart the corresponding service for changes to take effect.
                        </p>
                        <p className="text-xs mt-2" style={{ color: 'var(--text-secondary)' }}>
                            💾 <strong>Auto Backup:</strong> Config files are automatically backed up before editing. Backups are stored in <code className="px-1 py-0.5 rounded" style={{ background: 'var(--bg-tertiary)' }}>{dataPath.current || 'C:\\LocalDevine'}\\config\\backups</code>
                        </p>
                    </div>
                </div>

                {/* Log Files Quick Access */}
                <div className="card p-6 lg:col-span-2">
                    <div className="flex items-center gap-3 mb-6">
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-red-500 to-orange-600 flex items-center justify-center text-lg">
                            📋
                        </div>
                        <h2 className="text-xl font-heading" style={{ color: 'var(--text-on-card)' }}>Log Files</h2>
                    </div>

                    <p className="mb-4 text-sm" style={{ color: 'var(--text-secondary)' }}>
                        Quick access to service log files for debugging. Logs are located in <code className="px-1 py-0.5 rounded" style={{ background: 'var(--bg-secondary)' }}>{dataPath.current || 'C:\\LocalDevine'}\\logs</code>
                    </p>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <button
                            onClick={() => window.electronAPI?.openLogFile('apache-error')}
                            className="button-secondary text-left flex items-center gap-3 p-4"
                        >
                            <span className="text-2xl">❌</span>
                            <div className="flex-1">
                                <div className="font-semibold" style={{ color: 'var(--text-on-card)' }}>Apache Error Log</div>
                                <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>logs/apache/error.log</div>
                            </div>
                        </button>

                        <button
                            onClick={() => window.electronAPI?.openLogFile('apache-access')}
                            className="button-secondary text-left flex items-center gap-3 p-4"
                        >
                            <span className="text-2xl">📊</span>
                            <div className="flex-1">
                                <div className="font-semibold" style={{ color: 'var(--text-on-card)' }}>Apache Access Log</div>
                                <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>logs/apache/access.log</div>
                            </div>
                        </button>
                    </div>
                    ...
                    <div className="mt-4 p-3 rounded-lg" style={{ background: 'var(--bg-secondary)' }}>
                        <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
                            💡 <strong>Tip:</strong> Check log files if a service fails to start or encounters errors.
                        </p>
                    </div>
                </div>
